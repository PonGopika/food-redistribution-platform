...
function App() {
  ...
  useEffect(() => {
    ...
    if (savedId && token) {
      setUser({
        ...
        isVerified: localStorage.getItem('user_isVerified') === 'true'
      });
    }
  ...
  const shareImpact = () => {
    ...
    addToast("📋 Impact stats copied to clipboard!", 'info');
  };
  ...
  const registerUser = async () => {
    if (!agreeTerms) {
      return addToast("⚠️ You must agree to the Terms & Conditions.", 'error');
    }

    if (regRole === 'NGO' && !regNgoNumber) {
      return addToast("⚠️ NGOs must provide a Registration Number.", 'error');
    }
    ...
    } catch (err) { addToast("Registration failed.", 'error'); }
  };
  ...
  const deleteAccount = async () => {
    ...
    try {
      ...
      logout();
    } catch (err) { addToast("Error deleting account.", 'error'); }
  };
  ...
  const handleAuthSuccess = (data) => {
    ...
    localStorage.setItem('user_isVerified', user.isVerified);
    setRegPassword('');
    fetchMyHistory();
  };
  ...
  const logout = () => {
    ...
    const logout = () => {
      localStorage.clear();
      setUser({ id: null, name: '', role: '', phone: '', address: '', isVerified: false });
  ...
  const postDonation = async () => {
    if (user.role !== 'Donor') return addToast("Only Donors can post!", 'error');

    const qty = parseFloat(foodQty);
    const expiry = parseInt(foodExpiry);

    if (isNaN(qty) || qty <= 0) return addToast("⚠️ Quantity must be positive.", 'error');

    // FEATURE: Character Count Check
    if (foodDesc.length > 300) return addToast("⚠️ Description too long (max 300 chars).", 'error');
    ...
    } catch (err) { addToast("Error posting donation.", 'error'); }
  };
  ...
  const submitRating = async (id, ratingValue) => {
    ...
    try {
      ...
      fetchListings();
    } catch (err) { addToast("Error submitting rating.", 'error'); }
  };
  ...
  // FILTER LOGIC
  // ==========================================
  const sourceListings = view === 'history' ? myListings : listings;

  const visibleListings = sourceListings.filter(item => {
      ...
      <div className="card" style={currentCardStyle}>
        ...
        {visibleListings.map(item => (
          <div key={item._id || item.id} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <div>
                ...
                <span style={{ fontSize: '0.8em', color: '#888', display: 'block', marginTop: '3px' }}>
                  ...
                  {/* FEATURE: Verified Badge (Task 1.2.3) */}
                  {item.donor && item.donor.isVerified && (
                    <VerifiedBadge />
                  )}
            ...
            {/* CHAIN OF CUSTODY (Visible to All) */}
            {(item.claimedBy || item.collectedBy) && (
              <div style={{ marginBottom: '15px', padding: '10px', background: highContrast ? '#222' : '#f9f9f9', borderRadius: '8px', borderLeft: '4px solid #34495e' }}>
                <h5 style={{ margin: '0 0 5px 0', color: '#7f8c8d', textTransform: 'uppercase', fontSize: '0.75em' }}>Journey</h5>

                {item.claimedBy && (
                  <div style={{ fontSize: '0.9em', marginBottom: '4px' }}>
                    <strong style={{ color: '#2c3e50' }}>To:</strong> {item.claimedBy.name}
                    {item.claimedBy.isVerified && <VerifiedBadge size="0.9em" />}
                  </div>
                )}

                {item.collectedBy && (
                  <div style={{ fontSize: '0.9em' }}>
                    <strong style={{ color: '#2c3e50' }}>Through:</strong> {item.collectedBy.name} <span style={{ color: '#7f8c8d', fontSize: '0.85em' }}>(Volunteer)</span>
                  </div>
                )}
              </div>
            )}
              ...
              <div style={{ background: highContrast ? '#222' : '#f3e5f5', padding: '15px', borderRadius: '8px', margin: '15px 0', borderLeft: '5px solid #8e44ad' }}>
                ...
                {item.claimedBy && (
                  <p style={{ cursor: 'pointer', textDecoration: 'underline', color: '#27ae60' }} onClick={() => openMap(item.claimedBy.address)}>
                    ...
                    📍 To: {item.claimedBy.name} {item.claimedBy.ngoRegNumber && <VerifiedBadge />} (Click to Map)
                  </p>
                )}
              </div>
            )}
