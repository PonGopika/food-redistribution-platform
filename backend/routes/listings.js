...
router.get('/', async (req, res) => {
    try {
        ...
        const listings = await FoodListing.find(query)
            ...
            .populate('claimedBy', 'name phone address ngoRegNumber isVerified')
...
router.post('/', auth, async (req, res) => {
    try {
        ...
        const newActivity = new Activity({
            ...
            user: req.user.id,
            target: listing._id,
            targetModel: 'FoodListing',
            details: `New listing created: ${title}`
        });
...
router.put('/:id/status', auth, async (req, res) => {
    try {
        ...
        // LOG ACTIVITY
        let action = `LISTING_${status.toUpperCase().replace(' ', '_')}`;
        let details = `Listing status updated to ${status}`;

        if (status === 'Claimed') details = `Listing claimed by user`;
        if (status === 'In Transit') details = `Listing picked up for delivery`;
        if (status === 'Delivered') details = `Listing delivered successfully`;
        if (status === 'Cancelled') details = `Listing cancelled. Reason: ${reason}`;
